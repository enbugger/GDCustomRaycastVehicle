...
func shape_cast_v2():
	...
	if get_collision_count() == 0:
		...
		push_error("%d collisions detected" % get_collision_count())
	result.is_hit = true
	#result.hit_distance = get_closest_collision_safe_fraction() * target_position.length()
	result.hit_distance = (global_transform.origin + target_position - get_collision_point(0)).length()
	result.hit_position = get_collision_point(0)
	
	#DrawLine3D.DrawCube(global_transform.origin + target_position, 0.04, Color(255,0,0))
	DrawLine3D.DrawCube(get_collision_point(0), 0.04, Color(255,255,0))
	
	result.hit_normal = get_collision_normal(0)
	result.hit_point_velocity = Vector3.ZERO
	result.hit_body = null
	if get_collider_rid(0):
		result.hit_body = instance_from_id(PhysicsServer3D.body_get_object_instance_id(get_collider_rid(0)))
		var hitBodyState := PhysicsServer3D.body_get_direct_state(get_collider_rid(0))
		var hitBodyPoint : Vector3 = get_collision_point(0)
...
func apply_force(force : Vector3) -> void:
	if is_colliding():
		parentBody.apply_force(force, get_collision_point(0) - parentBody.global_transform.origin)

func _physics_process(delta) -> void:
	# perform sphere cast
	var castResult = shape_cast_v2()
...
#			suspensionForceVec = Vector3.UP * suspensionForce
		...
		# draw debug lines
		if GameState.debugMode:
			DrawLine3D.DrawRay(get_collision_point(0),suspensionForceVec/GameState.debugRayScaleFac,Color(0,255,0))
			DrawLine3D.DrawRay(get_collision_point(0),XForce/GameState.debugRayScaleFac,Color(255,0,0))
			DrawLine3D.DrawRay(get_collision_point(0),ZForce/GameState.debugRayScaleFac,Color(0,0,255))
			
		# apply forces relative to parent body
		var f:Vector3 = get_collision_point(0) - parentBody.global_transform.origin
		...
		if castResult.hit_body && castResult.hit_body is RigidBody3D:
			castResult.hit_body.apply_force(-finalForce, get_collision_point(0) - castResult.hit_body.global_transform.origin)
	...
	HIT DISTANCE: %s" % 
		[
			...
			get_collision_point(0),
			get_collision_normal(0),
...
#			suspensionForceVec = Vector3.UP * suspensionForce
		...
		# apply forces relative to parent body
		parentBody.apply_force(finalForce, get_collision_point(0) - parentBody.global_transform.origin)
